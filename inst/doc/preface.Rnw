...
of the \S{} programming language, which is powerful and flexible and has excellent 
...
@
We welcome comments on the \R{} package {\fontseries{b}\selectfont HSAUR}, and
...
code may differ a little from that given in the  {\fontseries{b}\selectfont HSAUR} package,
