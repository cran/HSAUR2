...
\begin{itemize}
\item copyright statements tables
\item adjust line width manually where necessary
\item HSAUR2 documentation update
\item provide slides for all chapters
\item solutions manual: we have model R code!
\end{itemize}
